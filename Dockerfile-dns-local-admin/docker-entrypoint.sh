...
if [ "$1" = "gunicorn" ] && [ ! -f /var/www/pdns-admin/powerdnsadmin/docker_config.py ]; then
    ...
    sed -i "s|SQLA_DB_HOST = '127.0.0.1'|SQLA_DB_HOST = '${ADMIN_DB_HOST:-admin-db}:${ADMIN_DB_PORT:-6432}'|g" /var/www/pdns-admin/powerdnsadmin/docker_config.py
    ...
    attempts=0
    while ! psql "host=${ADMIN_DB_HOST:-admin-db} dbname=${ADMIN_DB_NAME:-pda} user=${ADMIN_DB_USER:-pda} password=${ADMIN_DB_PASS:-pda} port=${ADMIN_DB_PORT:-6432}" -c 'select 1' >/dev/null 2>&1; do
