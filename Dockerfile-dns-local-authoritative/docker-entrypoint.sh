...
if [ "$1" = "pdns_server" ] && [ ! -f /etc/pdns/pdns.conf ]; then
    ...
    sed -i "s|# local-port=53|local-port=${AUTHORITATIVE_LOCAL_PORT:-2053}|g" /etc/pdns/pdns.conf
    ...
    sed -i "s|# allow-dnsupdate-from=127.0.0.0/8,::1|allow-dnsupdate-from=${AUTHORITATIVE_ALLOW_DNSUPDATE_FROM:-0.0.0.0/0,::/0}|g" /etc/pdns/pdns.conf
